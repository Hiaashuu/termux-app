...
public class SpecialButtonState {
    ...
    public void setIsActive(boolean value) {
        isActive = value;
        int defaultBtnBg = com.termux.shared.theme.ThemeUtils.getSystemAttrColor(mExtraKeysView.getContext(), com.termux.shared.R.attr.extraKeysButtonDefaultBg, 0x1AFFFFFF);
        for (MaterialButton button : buttons) {
            button.setTextColor(value ? mExtraKeysView.getButtonActiveTextColor() : mExtraKeysView.getButtonTextColor());
            button.setBackgroundTintList(android.content.res.ColorStateList.valueOf(value ? mExtraKeysView.getButtonActiveBackgroundColor() : defaultBtnBg));
        }
    }
