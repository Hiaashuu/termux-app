...
public class TermuxTerminalSessionActivityClient extends TermuxTerminalSessionClientBase {
    ...
    public void renameSession(final TerminalSession sessionToRename) {
        ...
        mActivity.getTerminalDialogState().requestRenameSession(sessionToRename);
    }
    ...
    public void checkForFontAndColors() {
        try {
            ...
            final Typeface newTypeface = (fontFile.exists() && fontFile.length() > 0) ? Typeface.createFromFile(fontFile) : Typeface.createFromAsset(mActivity.getAssets(), "font.ttf");
    ...
    public void updateBackgroundColor() {
        ...
        TerminalSession session = mActivity.getCurrentSession();
        if (session != null && session.getEmulator() != null) {
            int color = session.getEmulator().mColors.mCurrentColors[TextStyle.COLOR_INDEX_BACKGROUND];

            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                boolean isDark = (mActivity.getResources().getConfiguration().uiMode & android.content.res.Configuration.UI_MODE_NIGHT_MASK) == android.content.res.Configuration.UI_MODE_NIGHT_YES;
                int sysColorId = isDark ? android.R.color.system_neutral1_900 : android.R.color.system_neutral1_50;
                int fgColorId = isDark ? android.R.color.system_neutral1_50 : android.R.color.system_neutral1_900;
                color = mActivity.getResources().getColor(sysColorId, mActivity.getTheme());
                int fgColor = mActivity.getResources().getColor(fgColorId, mActivity.getTheme());
                session.getEmulator().mColors.mCurrentColors[TextStyle.COLOR_INDEX_BACKGROUND] = color;
                session.getEmulator().mColors.mCurrentColors[TextStyle.COLOR_INDEX_FOREGROUND] = fgColor;
            } else {
                boolean isDark = (mActivity.getResources().getConfiguration().uiMode & android.content.res.Configuration.UI_MODE_NIGHT_MASK) == android.content.res.Configuration.UI_MODE_NIGHT_YES;
                color = isDark ? 0xFF1C1B1F : 0xFFFFFBFE;
                int fgColor = isDark ? 0xFFE6E1E5 : 0xFF1C1B1F;
                session.getEmulator().mColors.mCurrentColors[TextStyle.COLOR_INDEX_BACKGROUND] = color;
                session.getEmulator().mColors.mCurrentColors[TextStyle.COLOR_INDEX_FOREGROUND] = fgColor;
            }

            mActivity.getWindow().getDecorView().setBackgroundColor(color);
            
            android.view.View leftDrawer = mActivity.findViewById(com.termux.R.id.left_drawer);
            if (leftDrawer != null) {
            ...
            if (insetsController != null) {
                boolean isLight = com.termux.terminal.TerminalColors.getPerceivedBrightnessOfColor(color) > 130;
