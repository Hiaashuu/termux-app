...
final class TermuxInstaller {
    ...
    public static byte[] loadZipBytes(Context context) {
        try {
            ...
            Logger.logWarn(LOG_TAG, "Failed to load libtermux-bootstrap.so, falling back to assets: " + t.getMessage());
            return loadZipBytesFromAssets(context);
        }
    }

    private static byte[] loadZipBytesFromAssets(Context context) {
        ...
        try (java.io.InputStream in = context.getAssets().open(assetName);
            ...
            Logger.logError(LOG_TAG, "Failed to load bootstrap zip from assets: " + assetName + " " + e.getMessage());
            return new byte[0];
        }
    }
