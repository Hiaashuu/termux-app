...
public final class TerminalView extends View {
     ...
     */
    public boolean deleteSelectedTextIfOnCurrentLine() {
        if (!isSelectingText()) return false;
        
        int[] sel = new int[4];
        getTextSelectionCursorController().getSelectors(sel);
        int y1 = sel[0];
        int y2 = sel[1];
        int x1 = sel[2];
        int x2 = sel[3];
        ...
        int logStart = cy;
        while (logStart > 0 && mEmulator.getScreen().getLineWrap(logStart - 1)) {
            logStart--;
        }
        int logEnd = cy;
        while (logEnd < mEmulator.mRows - 1 && mEmulator.getScreen().getLineWrap(logEnd)) {
            logEnd++;
        }
        
        int overlapY1 = Math.max(y1, logStart);
        int overlapY2 = Math.min(y2, logEnd);
        
        if (overlapY1 <= overlapY2) {
            int overlapX1 = (overlapY1 == y1) ? x1 : 0;
            int overlapX2 = (overlapY2 == y2) ? x2 : mEmulator.mColumns - 1;
            
            if (overlapY1 == overlapY2 && overlapX1 > overlapX2) return false;
            
            int rightArrows = 0;
            int leftArrows = 0;
            
            if (overlapY2 > cy || (overlapY2 == cy && overlapX2 >= cx)) {
                String textRight = mEmulator.getScreen().getSelectedText(cx, cy, overlapX2, overlapY2);
                rightArrows = countBaseCharacters(textRight);
            } else {
                String textLeft = mEmulator.getScreen().getSelectedText(overlapX2 + 1, overlapY2, cx - 1, cy);
                leftArrows = countBaseCharacters(textLeft);
            }
            
            String selText = mEmulator.getScreen().getSelectedText(overlapX1, overlapY1, overlapX2, overlapY2);
            int backspaces = countBaseCharacters(selText);
            
            if (leftArrows > 0) {
                StringBuilder sb = new StringBuilder(leftArrows * 3);
                for (int i = 0; i < leftArrows; i++) sb.append("\033[D");
                mTermSession.write(sb.toString());
            } else if (rightArrows > 0) {
                StringBuilder sb = new StringBuilder(rightArrows * 3);
                for (int i = 0; i < rightArrows; i++) sb.append("\033[C");
                mTermSession.write(sb.toString());
            }
            
            if (backspaces > 0) {
                StringBuilder sb = new StringBuilder(backspaces);
                for (int i = 0; i < backspaces; i++) sb.append("\177");
                mTermSession.write(sb.toString());
            }
    ...
    private int countBaseCharacters(String text) {
        int count = 0;
        if (text != null) {
            for (int i = 0; i < text.length(); ) {
                int cp = text.codePointAt(i);
                i += Character.charCount(cp);
                if (com.termux.terminal.WcWidth.width(cp) > 0) {
                    count++;
                }
            }
        }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (isSelectingText()) {
            if (keyCode == KeyEvent.KEYCODE_DEL && deleteSelectedTextIfOnCurrentLine()) {
                stopTextSelectionMode();
                return true;
            }
            stopTextSelectionMode();
    ...
    public void inputCodePoint(int eventSource, int codePoint, boolean controlDownFromEvent, boolean leftAltDownFromEvent) {
        ...
        if (isSelectingText()) {
            deleteSelectedTextIfOnCurrentLine();
            stopTextSelectionMode();
        }
